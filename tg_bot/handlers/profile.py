...
import asyncio
from database import get_user, save_user, get_user_rating, format_rating, get_referral_count
...
# ==========================================
# 👤 ОСОБИСТІ ДАНІ (ЗАХИЩЕНО ВІД NoneType)
# ==========================================
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    # 🛡 ЗАХИСТ: Визначаємо raw_phone
    raw_phone = message.contact.phone_number if message.contact else message.text

    # 🛡 ЗАХИСТ: Якщо юзер кинув стікер, raw_phone буде None.
    if not raw_phone:
        ...
        msg = await message.answer("⚠️ <b>Це не номер телефону!</b>\nНадішліть контакт або введіть цифри:", reply_markup=kb, parse_mode="HTML")
        await state.update_data(last_msg_id=msg.message_id)
        return
    ...
    # Авто-корекція
    if len(clean_phone) == 10 and clean_phone.startswith('0'): clean_phone = '+38' + clean_phone
    elif len(clean_phone) == 11 and clean_phone.startswith('80'): clean_phone = '+3' + clean_phone[1:]
...
# ==========================================
# 🚘 ДАНІ АВТОМОБІЛЯ (ЗАХИЩЕНО ВІД NoneType)
# ==========================================
...
async def process_model(message: types.Message, state: FSMContext):
    ...
    if not message.text:
        await update_or_send_msg(message.bot, message.chat.id, state, "⚠️ <b>Будь ласка, введіть текст.</b>\nНапишіть марку авто:", kb_back())
        return
...
async def process_color(message: types.Message, state: FSMContext):
    ...
    # 🛡 ЗАХИСТ: Перевірка на текст
    if not message.text:
        await update_or_send_msg(message.bot, message.chat.id, state, "⚠️ <b>Це не текст!</b>\nВкажіть колір (напр. Чорний):", kb_back())
        return
...
async def process_number(message: types.Message, state: FSMContext, bot: Bot):
    ...
    # 🛡 ЗАХИСТ: Перевірка на текст
    if not message.text:
        await update_or_send_msg(message.bot, message.chat.id, state, "⚠️ <b>Це не номер!</b>\nНапишіть номерний знак текстом:", kb_back())
        return
